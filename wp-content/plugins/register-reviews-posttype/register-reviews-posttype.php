 ...
 *
 * 		review 			=> The name of the post type, e.g. post, page. Should be all lowercase.
 *		Reviews 				=> The plural label for the post type. Shown in wp-admin.
 *		Review 			=> The singular label for the post type. Shown in wp-admin.
 *		star-filled 					=> The name of a dashicon to show next to the post type name.
...
/**
 * Plugin Name: Register Reviews Post Type
 * Description: This plugin registers a new post type called Review, [review]
 * Version: 2.0.0
 * Author: Adam Taylor
 * License: GPL2
 */
...
if ( !class_exists( 'RPT_review' ) ) {
    ...
    public $supports;
		...
		function __construct() {
			...
			$this->singular_label = 'Review';
			$this->plural_label = 'Reviews';
		...
		public function init() {
			...
			// No taxonomy needed for reviews — each one is independent.
		}
		...
		public function display_column_data( $column_name, $post_id ) {
			...
			switch ( $column_name ) {
				...
				case 'quote':
					...
					echo esc_html( mb_strlen( $quote ) > 80 ? mb_substr( $quote, 0, 80 ) . '…' : $quote );
					break;

				case 'active':
					$active = get_post_meta( $post_id, 'active', true );
					echo $active ? '✅ Active' : '— Hidden';
					break;

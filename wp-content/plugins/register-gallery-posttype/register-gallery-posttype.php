 ...
 * 
 * 		gallery 			=> The name of the post type, e.g. post, page. Should be all lowercase.
 *		Galleries 				=> The plural label for the post type. Shown in wp-admin.
 *		Gallery 			=> The singular label for the post type. Shown in wp-admin.
 *		images-alt2 					=> The name of a dashicon to show next to the post type name.
 ...
 * Description: This plugin registers a new post type called Gallery, [gallery]
 * Version: 2.0.0
 * Author: Adam Taylor
 * License: GPL2
 */
...
if ( !class_exists( 'RPT_gallery' ) ) {
    ...
    public $supports;
		...
		function __construct() {
			...
			$this->singular_label = 'Gallery';
			$this->plural_label = 'Galleries';
		...
		public function init() {
			...
			$this->_registerTaxonomy( 'gallery_type', 'Gallery Type', 'Gallery Types' );
		}
			...
			switch ( $column_name ) {
	          ...
	          <?php
				...
				case 'video_embed':
					...
					$video_embed = get_post_meta( $post_id, $column_name, true );
					if ( 'video' == $type && $video_embed ) {
	          
	          ?>
	          
	          <a href="<?php echo $video_embed; ?>" target="_blank" rel="noopener">
	          ...
	          <span aria-hidden="true">—</span>
	          <span class="screen-reader-text">No video embedded</span>
	          ...
	          <?php
				...
				case 'caption':
					// Create a list of possible captions
					// The first non-falsey value in the list will be echoed
					$captions = array();
					$captionsFrom = array();

					$captions[] = get_post_meta( $post_id, $column_name, true );
					$captionsFrom[] = sprintf( ' <a href="%s" target="_blank">[Custom]</a>', get_edit_post_link( $post_id ) );

					$thumbnail_id = get_post_meta( $post_id, 'image', true );
					$thumbnail_post = get_post( $thumbnail_id );
					$thumbnail_alt = get_post_meta( $thumbnail_id, '_wp_attachment_image_alt', true );

					$captions[] = $thumbnail_post->post_excerpt;
					$captionsFrom[] = sprintf( ' <a href="%s" target="_blank">[Media Library: Caption]</a>', get_edit_post_link( $thumbnail_id ) );
					
					$captions[] = $thumbnail_alt;
					$captionsFrom[] = sprintf( ' <a href="%s" target="_blank">[Media Library: Alt Text]</a>', get_edit_post_link( $thumbnail_id ) );

					// Find first non-falsey value
					foreach ($captions as $key => $value) {
						if ( $value ) {
							$caption = $value;
							$captionFrom = $captionsFrom[ $key ];
							break;
						}
					}

					echo $caption ? sprintf( '<strong>%s</strong>:<br>%s', $captionFrom, $caption ) : '—';
					break;
